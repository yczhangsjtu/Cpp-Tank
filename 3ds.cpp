...
void CLoad3DS::ProcessNextMaterialChunk(t3DModel *pModel, tChunk *pPreviousChunk)
{
	...
	while (pPreviousChunk->bytesRead < pPreviousChunk->length)
	{
		...
		default:  
			...
			// 掠过不需要读入的块
			m_CurrentChunk->bytesRead +=
				fread(buffer, 1, m_CurrentChunk->length - m_CurrentChunk->bytesRead, m_FilePointer);
			break;
		}
...
void CLoad3DS::ReadObjectMaterial(t3DModel *pModel, t3DObject *pObject, tChunk *pPreviousChunk)
{
	...
	pPreviousChunk->bytesRead +=
		fread(buffer, 1, pPreviousChunk->length - pPreviousChunk->bytesRead, m_FilePointer);
}			
...
void CreateTextures(UINT g_Texture[], t3DModel & g_3DModel)
{
	// 遍历所有的材质
...
void CreateTexture(UINT textureArray[], const char* strFileName, int textureID)
{
	...
	if(texPath==NULL)
	{
		pBitmap = stbi_load(strFileName, &x, &y, &n, 0);
	}
	else
	{
		char filename[256];
		strncpy(filename,texPath,255);
		strncat(filename,strFileName,255-strlen(filename));
		pBitmap = stbi_load(filename, &x, &y, &n, 0);
	}
	if(pBitmap == NULL)									// 如果装入位图失败，则返回
	{
		perror(strFileName);
		return;
	}
	...
	gluBuild2DMipmaps(GL_TEXTURE_2D, 3, x, y, GL_RGB, GL_UNSIGNED_BYTE, pBitmap);
	...
	{
		stbi_image_free(pBitmap);					
	}
}

void render3DModel(const t3DModel & g_3DModel, UINT g_Texture[])
{
	// 遍历模型中所有的对象
	for(int i = 0; i < g_3DModel.numOfObjects; i++)
	{
		...
		// 获得当前显示的对象
		const t3DObject *pObject = &g_3DModel.pObject[i];
		...
		for(int j = 0; j < pObject->numOfFaces; j++)
		{
			...
			for(int whichVertex = 0; whichVertex < 3; whichVertex++)
			{
				...
				if(pObject->bHasTexture) {
					...
					{
						const BYTE *pColor = g_3DModel.pMaterials[pObject->materialID].color;
